...
def generate_spsl(N,M,scale=False):
    datadir = 'data/spsl/%s_%s%s.hdf5'%(N,M,'_scaled' if scale else '')
    if os.path.isfile(datadir):
        return

    dt = h5py.File(datadir,'w')

    pi = np.random.uniform(0,0.5)
    mask = np.random.uniform(0,1,size=M) < pi
    ...
    if scale:
        ...
        X = np.apply_along_axis(lambda x: (x-np.mean(x))/np.std(x), 0, X) / np.sqrt(M*mask.mean())
        e = np.random.randn(N)*s_e
        y = X @ beta + e
        y -= np.mean(y)
    else:
        X = np.random.randn(M*N).reshape(N,M)
        X = np.apply_along_axis(lambda x: (x-np.mean(x))/np.std(x), 0, X)
        e = np.random.randn(N)*s_e*np.sqrt(M*mask.mean())
        y = X @ beta + e
        y -= np.mean(y)
...
if __name__ == '__main__':
    ...
    for packed in inds:
        ...
        N = N_array[i]
        M = M_array[j]
        generate_lmm(N,M,sys.argv[-2] == 'True')
